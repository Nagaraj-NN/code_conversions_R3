...
{{ config(
    materialized='table',
    alias='FEL_FACILITY_DIM_SRC',
    meta={"mapping_name": "m_FEL_COMTRAC_FACILITY_DIM_ins_upd", "workflow_name": "wkf_FEL_COAL_DIM_LOAD", "session_name": "s_m_FEL_COMTRAC_FACILITY_DIM_ins_upd"},
    pre_hook=[
        log_model_start(this, 'TBD_FEL_FACILITY_DIM', target_object='GENDB01.FELADM.FEL_FACILITY_DIM')
    ],
    post_hook=[
        "UPDATE {{ source('GENDB01_FELADM','FEL_FACILITY_DIM') }} TGT
     SET
     ...
     WHERE TGT.FACILITY_ID = SRC.FACILITY_ID
       ...
       AND (   SRC.FACILITY_NM      <> TGT.FACILITY_NM
        ...
        "INSERT INTO {{ source('GENDB01_FELADM','FEL_FACILITY_DIM') }} (
         FACILITY_KEY,
         FACILITY_ID,
         FACILITY_NM,
         FCLTY_TYPE_NM,
     ...
     WHERE NOT EXISTS (
         SELECT 1
         FROM {{ source('GENDB01_FELADM','FEL_FACILITY_DIM') }} L
         WHERE L.FACILITY_ID = SRC.FACILITY_ID
           AND L.SYSTEM_ID   = SRC.SYSTEM_ID
     )",
        log_model_end(this, 'TBD_FEL_FACILITY_DIM', target_object='GENDB01.FELADM.FEL_FACILITY_DIM')
    ]
) }}
...
FROM (
        SELECT
            ...
            NVL((SELECT MAX(FACILITY_KEY) FROM {{ source('GENDB01_FELADM','FEL_FACILITY_DIM') }}), 0)
                + ROW_NUMBER() OVER (ORDER BY SQ.FACILITY_ID)                                  AS O_FACILITY_KEY
        FROM {{ source('GENDB01_FELADM','FEL_COMTRAC_FACILITY_FDR') }} SQ
        LEFT JOIN (
          ...
          ON XRF.DATA_TYP_ID = (SELECT MIN(DATA_TYPE_ID) FROM {{ source('GENDB01_FELADM','FEL_DATATYPE_DIM') }} WHERE DATA_TYPE_NM = 'FACILITY')
