-- ==========================================================================
-- Model      : FEL_PWRMGR_LOAD_TYPE_FDR
-- Mapping    : m_FEL_PWRMGR_LOAD_TYPE_FDR_ins
-- Workflow   : wkf_FEL_STATIC_DIM_LOAD
-- Session    : s_m_FEL_PWRMGR_LOAD_TYPE_FDR_ins
-- Target     : GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR
-- Load type  : table + UPDATE post-hook + INSERT post-hook
-- --------------------------------------------------------------------------
-- UPDATE ELSE INSERT on LOAD_TYPE_ID. Treat source rows as Update with the
-- writer set to Update else Insert, so a row is updated when the id exists
-- and inserted when it does not.
--
...
{{ config(
    materialized='table',
    alias='FEL_PWRMGR_LOAD_TYPE_FDR_SRC',
    meta={"mapping_name": "m_FEL_PWRMGR_LOAD_TYPE_FDR_ins", "workflow_name": "wkf_FEL_STATIC_DIM_LOAD", "session_name": "s_m_FEL_PWRMGR_LOAD_TYPE_FDR_ins"},
    pre_hook=[
        log_model_start(this, 'TBD_FEL_PWRMGR_LOAD_TYPE_FDR', target_object='GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR')
    ],
    post_hook=[
        "UPDATE {{ source('GENDB01_FELADM','FEL_PWRMGR_LOAD_TYPE_FDR') }} TGT
     SET
         LOAD_TYPE_NM = SRC.LOAD_TYPE_NAME,
         LAST_UPDT_TS = SRC.LAST_UPDT_TS
     FROM {{ this }} SRC
     WHERE TGT.LOAD_TYPE_ID = SRC.LOAD_TYPE_ID",
        "INSERT INTO {{ source('GENDB01_FELADM','FEL_PWRMGR_LOAD_TYPE_FDR') }} (
         LOAD_TYPE_ID,
         LOAD_TYPE_NM,
         LAST_UPDT_TS
     )
     ...
     WHERE NOT EXISTS (
         SELECT 1
         FROM {{ source('GENDB01_FELADM','FEL_PWRMGR_LOAD_TYPE_FDR') }} L
         WHERE L.LOAD_TYPE_ID = SRC.LOAD_TYPE_ID
     )",
        log_model_end(this, 'TBD_FEL_PWRMGR_LOAD_TYPE_FDR', target_object='GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR')
    ]
) }}
...
FROM (
        ...
        FROM (
            ...
            FROM (
                ...
                FROM {{ source('GENDB01_DEV_SANDBOX_COMMON','ENUMTABLE_TEST') }} SQ  --USED FOR TESTING
                WHERE SQ.ENUMID = 82
            ) DQ
        ) P
        CROSS JOIN (
