-- ==========================================================================
-- Model      : FEL_COAL_RCPTS_QTY_QLTY_FDR
...
{{ config(
    materialized='table',
    alias='FEL_COAL_RCPTS_QTY_QLTY_FDR_SRC',
    meta={"mapping_name": "m_FEL_COAL_RCPTS_QTY_QLTY_FDR_Cmtrt_ins_upd", "workflow_name": "wkf_FEL_CMTRT_FDR_LOAD", "session_name": "s_m_FEL_COAL_RCPTS_QTY_QLTY_FDR_Cmtrt_ins_upd"},
    pre_hook=[
        log_model_start(this, 'TBD_FEL_COAL_RCPTS_QTY_QLTY_FDR', target_object='GENDB01.FELADM.FEL_COAL_RCPTS_QTY_QLTY_FDR')
    ],
    post_hook=[
        "UPDATE {{ source('GENDB01_FELADM','FEL_COAL_RCPTS_QTY_QLTY_FDR') }} TGT
     SET
     ...
     WHERE TGT.RECEIPT_ID = SRC.RECEIPT_ID",
        "INSERT INTO {{ source('GENDB01_FELADM','FEL_COAL_RCPTS_QTY_QLTY_FDR') }} (
         RECEIPT_ID,
         CMDTY_SRC_ID,
         SHIPPED_TS,
         UNLOADD_TS,
     ...
     WHERE NOT EXISTS (
         SELECT 1
         FROM {{ source('GENDB01_FELADM','FEL_COAL_RCPTS_QTY_QLTY_FDR') }} LKP
         WHERE LKP.RECEIPT_ID = SRC.RECEIPT_ID
     )",
        log_model_end(this, 'TBD_FEL_COAL_RCPTS_QTY_QLTY_FDR', target_object='GENDB01.FELADM.FEL_COAL_RCPTS_QTY_QLTY_FDR')
    ]
) }}
...
FROM (
        ...
        FROM {{ source('GENDB01_DEV_SANDBOX_COMMON','AEP_DW_RECEIPT_VW_TEST') }} SQ  --USED FOR TESTING
