-- ==========================================================================
-- Model      : FEL_CNTRCT_MNTHY_SMRY_FACT
...
-- Target     : GENDB01.FELADM.FEL_CNTRCT_MNTHY_SMRY_FACT
...
{{ config(
    materialized='table',
    alias='FEL_CNTRCT_MNTHY_SMRY_FACT_SRC',
    meta={"mapping_name": "m_FEL_CNTRCT_MNTHY_SMRY_FACT_ins_upd", "workflow_name": "wkf_FEL_COAL_FACT_LOAD", "session_name": "s_m_FEL_CNTRCT_MNTHY_SMRY_FACT_ins_upd"},
    pre_hook=[
        log_model_start(this, 'TBD_FEL_CNTRCT_MNTHY_SMRY_FACT', target_object='GENDB01.FELADM.FEL_CNTRCT_MNTHY_SMRY_FACT')
    ],
    post_hook=[
        "UPDATE {{ source('GENDB01_FELADM','FEL_CNTRCT_MNTHY_SMRY_FACT') }} TGT
     SET
     ...
     WHERE TGT.CNTRCT_PROD_KEY = SRC.CNTRCT_PROD_KEY
       ...
       AND (   SRC.CONTRACT_KEY     <> TGT.CONTRACT_KEY
        ...
        "INSERT INTO {{ source('GENDB01_FELADM','FEL_CNTRCT_MNTHY_SMRY_FACT') }} (
         CNTRCT_PROD_KEY,
         GEOG_FCLTY_KEY,
         OPRTG_MO_DAY_ID,
         CONTRACT_KEY,
     ...
     WHERE NOT EXISTS (
         SELECT 1
         FROM {{ source('GENDB01_FELADM','FEL_CNTRCT_MNTHY_SMRY_FACT') }} L
        ...
        log_model_end(this, 'TBD_FEL_CNTRCT_MNTHY_SMRY_FACT', target_object='GENDB01.FELADM.FEL_CNTRCT_MNTHY_SMRY_FACT')
    ]
) }}
...
FROM (
    ...
    LEFT JOIN (
     ...
     AND LKP_FD.SYSTEM_ID   = (SELECT MIN(SYS_ID) FROM {{ source('GENDB01_FELADM','FEL_SYSTEM_DIM') }} WHERE SYS_NM = 'COMTRAC')
    LEFT JOIN (
    ...
    LEFT JOIN (
     ...
     AND LKP_CN.SYSTEM_ID   = (SELECT MIN(SYS_ID) FROM {{ source('GENDB01_FELADM','FEL_SYSTEM_DIM') }} WHERE SYS_NM = 'COMTRAC')
    LEFT JOIN (
