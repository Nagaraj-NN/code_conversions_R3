    ...
    {% if execute and target.name in ('qa', 'uat', 'prod') %}
        ...
        {% if (autosys_job_name | string).startswith('TBD') %}
            {{ exceptions.raise_compiler_error(
                ...
                ~ "'. Set the real Autosys job name in log_model_start,"
                ~ " log_model_end and seeds/FEL_JOB_CONTROL_SEEDS.csv - all three"
                ~ " must agree or the control row never matches."
            ) }}
        {% endif %}
