...
--------------------------------------------------------------------------------
 SOURCE    : feladm.FEL_COMTRAC_BSNS_ENTY_FDR   source filter on time
 LOOKUPS   : seven, all Use Any Value. FEL_SYSTEM_DIM $$COMTRAC, FEL_DATATYPE_DIM
             $$BUSINESSENTITY, MAX(CONSOLIDATION_ID) and MAX(BSNS_ENTY_KEY) are
             unconnected; FEL_MASTER_DIM_XRF twice, for the consolidation id and
             for the master record, and FEL_BSNS_ENTY_DIM, are connected.
 TARGET    : feladm.FEL_BSNS_ENTY_DIM   insert and update instances
--------------------------------------------------------------------------------
 PARAMETERS: $$START_TIME, $$RUN_STATUS (SQ filter), $$COMTRAC, $$BUSINESSENTITY
             $$REC_STATUS and $$FUELWORKS are declared but not referenced
 ...
 SQ OVERRIDE : none. The filter carries a COMMENTED OUT trailing clause
               --and UPPER(TRIM(STATUS_TX)) = 'ACTIVE', not reproduced below.
 PRE-SQL   : none     POST-SQL : none     TRUNCATE TARGET : no
--------------------------------------------------------------------------------
 NOTES     : CONSOLIDATION_ID is the cross reference id for this entity, or the
             ...
             entity's own name and 'COMTRAC' when there is no master row.
             AP_VNDR_ID, BSNS_ENTY_OLD_NM and LDC_CD are the constant 'N/A'.
             The counter behind both the entity key and a new consolidation id
             increments once per row read, UPSTREAM of the router.
...
SET V_BUSINESSENTITY = 'BUSINESSENTITY';
SET V_SESSSTARTTIME  = CAST(CURRENT_TIMESTAMP() AS TIMESTAMP_NTZ);

UPDATE feladm.FEL_BSNS_ENTY_DIM TGT
SET
...
FROM (
    ...
    FROM (
        SELECT
            ...
            NVL((SELECT MAX(BSNS_ENTY_KEY) FROM FELADM.FEL_BSNS_ENTY_DIM), 0)
                + ROW_NUMBER() OVER (ORDER BY SQ.BSNS_ENTY_ID)                                 AS O_BSNS_ENTY_KEY
        FROM feladm.FEL_COMTRAC_BSNS_ENTY_FDR SQ
        LEFT JOIN (
          ...
          ON XRF.DATA_TYP_ID = (SELECT MIN(DATA_TYPE_ID) FROM feladm.FEL_DATATYPE_DIM WHERE DATA_TYPE_NM = $V_BUSINESSENTITY)
...
INSERT INTO feladm.FEL_BSNS_ENTY_DIM (
    BSNS_ENTY_KEY,
    BSNS_ENTY_ID,
    STATUS_TX,
    BSNS_ENTY_TYPE_NM,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM feladm.FEL_BSNS_ENTY_DIM L
