...
--------------------------------------------------------------------------------
 SOURCE    : pmgr.ENUMTABLE
             --  GENDB01_DEV_SANDBOX.COMMON.ENUMTABLE_TEST (USED FOR TESTING)
 LOOKUPS   : GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR
...
--------------------------------------------------------------------------------
 ...
 PRE-SQL   : none     POST-SQL : none     TRUNCATE TARGET : no
    $Source connection value: INTEGMQ
...
UPDATE GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR TGT
SET
    ...
    FROM (
        ...
        FROM (
            ...
            FROM (
                ...
                FROM GENDB01_DEV_SANDBOX.COMMON.ENUMTABLE_TEST SQ  --USED FOR TESTING
                WHERE SQ.ENUMID = 82
            ) DQ
        ) P
        CROSS JOIN (
...
INSERT INTO GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR (
    LOAD_TYPE_ID,
    LOAD_TYPE_NM,
    LAST_UPDT_TS
)
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR L
