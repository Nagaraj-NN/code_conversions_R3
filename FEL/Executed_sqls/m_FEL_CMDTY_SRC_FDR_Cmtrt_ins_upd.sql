...
--------------------------------------------------------------------------------
 SOURCE    : AEP_DW_COMMODITYSOURCE_VW (COMTRACP db)
             GENDB01_DEV_SANDBOX.COMMON.AEP_DW_COMMODITYSOURCE_VW_TEST (USED FOR TESTING)
 LOOKUP    : feladm.FEL_CMDTY_SRC_FDR 
             GENDB01.FELADM.FEL_CMDTY_SRC_FDR  (USED FOR TESTING)
...
--------------------------------------------------------------------------------
 ...
 PRE-SQL   : none     POST-SQL : none     TRUNCATE TARGET : no
    ...
    $DBConnection_Source=fel_COMTRAC (COMTRACP db)
...
UPDATE GENDB01.FELADM.FEL_CMDTY_SRC_FDR TGT
SET
...
FROM (
    ...
    FROM (
        SELECT
            ...
            SQ.ALLOC_EFF_DT,
            SQ.ALLOC_EXP_DT
        FROM GENDB01_DEV_SANDBOX.COMMON.AEP_DW_COMMODITYSOURCE_VW_TEST SQ  --USED FOR TESTING
...
INSERT INTO GENDB01.FELADM.FEL_CMDTY_SRC_FDR (
    ALLOCATED_ID,
    CMDTY_SRC_ID,
    CMDTY_SRC_EFCTV_DT,
    CMDTY_SRC_EXPR_DT,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_CMDTY_SRC_FDR LKP
