...
--------------------------------------------------------------------------------
 SOURCE    : GENDB01.FELADM.FEL_COMTRAC_BSNS_ENTY_FDR
...
UPDATE GENDB01.FELADM.FEL_BSNS_ENTY_DIM TGT
SET
...
FROM (
    ...
    FROM (
        SELECT
            ...
            NVL((SELECT MAX(BSNS_ENTY_KEY) FROM GENDB01.FELADM.FEL_BSNS_ENTY_DIM), 0)
                + ROW_NUMBER() OVER (ORDER BY SQ.BSNS_ENTY_ID)                                 AS O_BSNS_ENTY_KEY
        FROM GENDB01.FELADM.FEL_COMTRAC_BSNS_ENTY_FDR SQ
        LEFT JOIN (
          ...
          ON XRF.DATA_TYP_ID = (SELECT MIN(DATA_TYPE_ID) FROM GENDB01.FELADM.FEL_DATATYPE_DIM WHERE DATA_TYPE_NM = $V_BUSINESSENTITY)
...
INSERT INTO GENDB01.FELADM.FEL_BSNS_ENTY_DIM (
    BSNS_ENTY_KEY,
    BSNS_ENTY_ID,
    STATUS_TX,
    BSNS_ENTY_TYPE_NM,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_BSNS_ENTY_DIM L
