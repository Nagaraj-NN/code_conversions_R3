...
--------------------------------------------------------------------------------
 SOURCES   : GENDB01.FELADM.FEL_COMTRAC_CONTRACT_FDR
...
UPDATE GENDB01.FELADM.FEL_CONTRACT_DIM TGT
SET
...
FROM (
    ...
    FROM (
        SELECT
            ...
            (SELECT MIN(SYS_ID) FROM GENDB01.FELADM.FEL_SYSTEM_DIM WHERE SYS_NM = 'COMTRAC')                           AS V_SYS_ID,
            NVL((SELECT MAX(CONTRACT_KEY) FROM GENDB01.FELADM.FEL_CONTRACT_DIM), 0)
                + ROW_NUMBER() OVER (ORDER BY SQ.CONTRACT_ID)                                                  AS O_CONTRACT_KEY
        FROM GENDB01.FELADM.FEL_COMTRAC_CONTRACT_FDR SQ
...
INSERT INTO GENDB01.FELADM.FEL_CONTRACT_DIM (
    CONTRACT_KEY,
    CONTRACT_ID,
    CNTRCT_EFCTV_DT,
    CNTRCT_EXPR_DT,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_CONTRACT_DIM L
