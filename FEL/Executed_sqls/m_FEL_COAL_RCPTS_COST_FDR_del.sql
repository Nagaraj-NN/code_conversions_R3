...
--------------------------------------------------------------------------------
 SOURCE    : GENDB01.FELADM.FEL_COAL_RCPTS_COST_FDR   (the target read back through itself)
 LOOKUP    : none
...
DELETE FROM GENDB01.FELADM.FEL_COAL_RCPTS_COST_FDR TGT
WHERE EXISTS (
    SELECT 1
    FROM (
        SELECT
        ...
        FROM GENDB01.FELADM.FEL_COAL_RCPTS_COST_FDR
