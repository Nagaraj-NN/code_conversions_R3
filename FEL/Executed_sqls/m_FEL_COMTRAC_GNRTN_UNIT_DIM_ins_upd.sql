...
--------------------------------------------------------------------------------
 SOURCE    : GENDB01.FELADM.FEL_COMTRAC_GNRTN_UNIT_FDR
 LOOKUPS   : GENDB01.FELADM.FEL_SYSTEM_DIM
             GENDB01.FELADM.FEL_DATATYPE_DIM
             GENDB01.FELADM.FEL_MASTER_DIM_XRF
             GENDB01_DEV_SANDBOX.COMMON.AEP_DW_GENUNIT_VW
...
UPDATE GENDB01.FELADM.FEL_GNRTN_UNIT_DIM TGT
SET
    UNIT_ID          = SRC.UNIT_ID,
    UNIT_NM          = SRC.UNIT_NM,
...
FROM (
    ...
    FROM (
        SELECT
            ...
            IFF(XRF.CONSOLIDATION_ID IS NOT NULL, XRF.CONSOLIDATION_ID,
                NVL((SELECT MAX(CONSOLIDATION_ID) FROM GENDB01.FELADM.FEL_MASTER_DIM_XRF), 0)
                + ROW_NUMBER() OVER (ORDER BY SQ.UNIT_ID))                                 AS O_CONSOLIDATION_ID,
            NVL((SELECT MAX(UNIT_KEY) FROM GENDB01.FELADM.FEL_GNRTN_UNIT_DIM), 0)
                + ROW_NUMBER() OVER (ORDER BY SQ.UNIT_ID)                                  AS O_GEN_UNIT_KEY
        FROM GENDB01.FELADM.FEL_COMTRAC_GNRTN_UNIT_FDR SQ
        LEFT JOIN (
            SELECT FCLTY_ID, UNIT_ID
            FROM GENDB01_DEV_SANDBOX.COMMON.AEP_DW_GENUNIT_VW_TEST
        ...
        LEFT JOIN (
            ...
            FROM GENDB01.FELADM.FEL_MASTER_DIM_XRF xrf
          ...
          ON XRF.DATA_TYP_ID = (SELECT MIN(DATA_TYPE_ID) FROM GENDB01.FELADM.FEL_DATATYPE_DIM WHERE DATA_TYPE_NM = $V_GEN_UNIT)
         AND XRF.SYSTEM_ID   = (SELECT MIN(SYS_ID) FROM GENDB01.FELADM.FEL_SYSTEM_DIM WHERE SYS_NM = $V_COMTRAC)
    ...
    LEFT JOIN (
        ...
        FROM GENDB01.FELADM.FEL_MASTER_DIM_XRF XRF
        INNER JOIN GENDB01.FELADM.FEL_SYSTEM_DIM SYS
...
INSERT INTO GENDB01.FELADM.FEL_GNRTN_UNIT_DIM (
    UNIT_KEY,
    UNIT_ID,
    UNIT_NM,
    UNIT_TYP_CD,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_GNRTN_UNIT_DIM L
