...
--------------------------------------------------------------------------------
 SOURCE    : GENDB01.FELADM.FEL_INVTRY_LOC_FDR 
...
UPDATE GENDB01.FELADM.FEL_INVTRY_LOC_DIM TGT
SET
...
FROM (
    ...
    FROM (
        SELECT
            ...
            NVL((SELECT MAX(INVTRY_LOC_KEY) FROM GENDB01.FELADM.FEL_INVTRY_LOC_DIM), 0)
                + ROW_NUMBER() OVER (ORDER BY SQ.INVTRY_LOC_ID)                            AS O_INV_LOC_KEY
        FROM GENDB01.FELADM.FEL_INVTRY_LOC_FDR SQ
        LEFT JOIN (
          ...
          ON XRF.DATA_TYP_ID = (SELECT MIN(DATA_TYPE_ID) FROM GENDB01.FELADM.FEL_DATATYPE_DIM WHERE DATA_TYPE_NM = $V_INVTRYLOC)
...
INSERT INTO GENDB01.FELADM.FEL_INVTRY_LOC_DIM (
    INVTRY_LOC_KEY,
    INVTRY_LOC_ID,
    INVTRY_LOC_NM,
    INVTRY_LOC_TYPE_NM,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_INVTRY_LOC_DIM L
