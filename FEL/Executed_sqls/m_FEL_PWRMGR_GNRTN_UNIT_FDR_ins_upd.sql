...
--------------------------------------------------------------------------------
 SOURCE    : PMGR.UNIT
             --  GENDB01_DEV_SANDBOX.COMMON.UNIT_TEST (USED FOR TESTING)
             PMGR.UNIT_TYPE
             -- GENDB01_DEV_SANDBOX.COMMON.UNIT_TYPE_TEST
             PMGR.UTILITY_OPERATOR
             -- GENDB01_DEV_SANDBOX.COMMON.UTILITY_OPERATOR_TEST
             PMGR.CAP_UNIT_CAPABILITY
             -- GENDB01_DEV_SANDBOX.COMMON.CAP_UNIT_CAPABILITY_TEST
 LOOKUPS   : GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR
             PMGR.FUEL_CODE
             -- GENDB01_DEV_SANDBOX.COMMON.FUEL_CODE_TEST
             GENDB01.FELADM.FEL_FUEL_CODE_FDR
 TARGET    : GENDB01.FELADM.FEL_PWRMGR_GNRTN_UNIT_FDR   insert and update instances
--------------------------------------------------------------------------------
...
UPDATE GENDB01.FELADM.FEL_PWRMGR_GNRTN_UNIT_FDR TGT
SET
    UNIT_NM           = SRC.UNIT_NM,
...
FROM (
    ...
    FROM (
        ...
        FROM GENDB01_DEV_SANDBOX.COMMON.UNIT_TEST U  --USED FOR TESTING
        INNER JOIN (
            SELECT UNITID, MAX(EFFECTIVEDATE) AS m_EFFECTIVEDATE
            FROM GENDB01_DEV_SANDBOX.COMMON.UNIT_TEST
            GROUP BY UNITID
        ) m
          ON u.unitid = m.unitid
         AND u.EFFECTIVEDATE = m.m_EFFECTIVEDATE
        LEFT OUTER JOIN GENDB01_DEV_SANDBOX.COMMON.UNIT_TYPE_TEST ut
          ON u.UNITTYPECODE = ut.unittypecode
        LEFT OUTER JOIN GENDB01_DEV_SANDBOX.COMMON.UTILITY_OPERATOR_TEST uo
          ON u.UTILITY = uo.utility
    ) SQ
    LEFT JOIN (
        SELECT CAP1, UNITID
        FROM GENDB01_DEV_SANDBOX.COMMON.CAP_UNIT_CAPABILITY_TEST
    ...
    LEFT JOIN (
        ...
        FROM GENDB01.FELADM.FEL_PWRMGR_LOAD_TYPE_FDR
    ...
    LEFT JOIN (SELECT FC.DESCRIPTION, TRIM(FC.CODE) AS CODE FROM GENDB01_DEV_SANDBOX.COMMON.FUEL_CODE_TEST FC
    ...
    LEFT JOIN (SELECT F.FUEL_CD_DESCN_TX, UPPER(TRIM(F.FUEL_CD)) AS FUEL_CD FROM GENDB01.FELADM.FEL_FUEL_CODE_FDR F
...
INSERT INTO GENDB01.FELADM.FEL_PWRMGR_GNRTN_UNIT_FDR (
    UNIT_ID,
    UNIT_NM,
    UNIT_TYPE_CD,
    UNIT_TYPE_NM,
...
FROM (
    ...
    FROM (
        SELECT
            ...
            ut.unittype        AS unittype,
            uo.utilityname     AS businessname
        FROM GENDB01_DEV_SANDBOX.COMMON.UNIT_TEST U
        INNER JOIN (
            SELECT UNITID, MAX(EFFECTIVEDATE) AS m_EFFECTIVEDATE
            FROM GENDB01_DEV_SANDBOX.COMMON.UNIT_TEST
            GROUP BY UNITID
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_PWRMGR_GNRTN_UNIT_FDR U
