...
--------------------------------------------------------------------------------
 SOURCE    : AEP_DW_CONSUMPTION_VW
             GENDB01_DEV_SANDBOX.COMMON.AEP_DW_CONSUMPTION_VW_TEST  (USED FOR TESTING)
 LOOKUP    : GENDB01.FELADM.FEL_COAL_CNSMN_FDR   existence, RECLAIM_ID = CONSUMP_ID
             no lookup SQL override
 TARGET    : GENDB01.FELADM.FEL_COAL_CNSMN_FDR   insert and update instances
--------------------------------------------------------------------------------
...
UPDATE GENDB01.FELADM.FEL_COAL_CNSMN_FDR TGT
SET
    UNIT_ID              = SRC.UNIT_ID,
...
FROM (
    ...
    FROM (
        SELECT
            ...
            SQ.SAMPLED_TONS,
            SQ.CONSUMED_AMT
        FROM GENDB01_DEV_SANDBOX.COMMON.AEP_DW_CONSUMPTION_VW_TEST SQ  --USED FOR TESTING
...
INSERT INTO GENDB01.FELADM.FEL_COAL_CNSMN_FDR (
    RECLAIM_ID,
    UNIT_ID,
    INVTRY_LOC_ID,
    CNSMN_DATE,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_COAL_CNSMN_FDR LKP
