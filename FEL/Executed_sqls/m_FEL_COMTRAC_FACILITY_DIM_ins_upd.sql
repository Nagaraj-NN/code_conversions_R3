...
--------------------------------------------------------------------------------
 SOURCE    : GENDB01.FELADM.FEL_COMTRAC_FACILITY_FDR
...
UPDATE GENDB01.FELADM.FEL_FACILITY_DIM TGT
SET
...
FROM (
    ...
    FROM (
        SELECT
            ...
            NVL((SELECT MAX(FACILITY_KEY) FROM GENDB01.FELADM.FEL_FACILITY_DIM), 0)
                + ROW_NUMBER() OVER (ORDER BY SQ.FACILITY_ID)                                  AS O_FACILITY_KEY
        FROM GENDB01.FELADM.FEL_COMTRAC_FACILITY_FDR SQ
        LEFT JOIN (
          ...
          ON XRF.DATA_TYP_ID = (SELECT MIN(DATA_TYPE_ID) FROM GENDB01.FELADM.FEL_DATATYPE_DIM WHERE DATA_TYPE_NM = $V_FACILITY)
...
INSERT INTO GENDB01.FELADM.FEL_FACILITY_DIM (
    FACILITY_KEY,
    FACILITY_ID,
    FACILITY_NM,
    FCLTY_TYPE_NM,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_FACILITY_DIM L
