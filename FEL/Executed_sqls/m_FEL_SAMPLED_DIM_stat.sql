...
--------------------------------------------------------------------------------
 SOURCE    : COMTRACAPP.PERMVALUE
             --  GENDB01_DEV_SANDBOX.COMMON.PERMVALUE_TEST (USED FOR TESTING)
 LOOKUP    : GENDB01.FELADM.FEL_SAMPLED_DIM
 TARGET    : GENDB01.FELADM.FEL_SAMPLED_DIM   insert and update instances
--------------------------------------------------------------------------------
 ...
 PRE-SQL   : none     POST-SQL : none     TRUNCATE TARGET : no
    $Source connection value: fel_COMTRAC
    $Target connection value: fel_GENDB001
    db: COMTRACP
    Schema: FELADM
================================================================================
*/
...
UPDATE GENDB01.FELADM.FEL_SAMPLED_DIM TGT
SET
...
FROM (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM GENDB01_DEV_SANDBOX.COMMON.PERMVALUE_TEST t  --USED FOR TESTING
            WHERE t.tablename = 'sampleproblem'
              AND t.colname   = 'missedsamplereason'
            UNION
            SELECT 'Sampled'
...
INSERT INTO GENDB01.FELADM.FEL_SAMPLED_DIM (
    SAMPLED_KEY,
    SMPLD_RSN_TX,
    SAMPLED_TX,
    LAST_UPDT_TS
...
SELECT
    NVL((SELECT MAX(SAMPLED_KEY) FROM GENDB01.FELADM.FEL_SAMPLED_DIM), 0)
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_SAMPLED_DIM S
