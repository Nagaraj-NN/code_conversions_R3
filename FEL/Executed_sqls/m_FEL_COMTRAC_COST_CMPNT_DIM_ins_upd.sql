...
--------------------------------------------------------------------------------
 SOURCE    : FLAT FILE
             @FEL_INBOUND_STAGE/Cost_Type_Dim_Data.csv
             GENDB01_DEV_SANDBOX.COMMON.COST_TYPE_DIM_DATA_TEST (USED FOR TESTING)
 LOOKUPS   : GENDB01.FELADM.FEL_SYSTEM_DIM
             GENDB01.FELADM.FEL_COST_CMPNT_DIM
 TARGET    : GENDB01.FELADM.FEL_COST_CMPNT_DIM   insert and update instances
--------------------------------------------------------------------------------
...
UPDATE GENDB01.FELADM.FEL_COST_CMPNT_DIM TGT
SET
...
FROM (
    ...
    FROM (
        SELECT
            ...
            (SELECT MIN(SYS_ID) FROM GENDB01.FELADM.FEL_SYSTEM_DIM WHERE SYS_NM = $V_COMTRAC)    AS V_SYS_ID
        FROM (
            SELECT
            ...
            FROM GENDB01_DEV_SANDBOX.COMMON.COST_TYPE_DIM_DATA_TEST  --USED FOR TESTING
...
INSERT INTO GENDB01.FELADM.FEL_COST_CMPNT_DIM (
    CST_CMPNT_KEY,
    CST_CMPNT_ID,
    CST_CMPNT_NM,
    CST_CMPNT_CTGY_NM,
...
FROM (
    SELECT
        ...
        LEFT($V_COMTRAC, 25)                                                             AS SYSTEM_NM,
        $V_SESSSTARTTIME                                                                 AS LAST_UPDT_TS,
        NVL((SELECT MAX(CST_CMPNT_KEY) FROM GENDB01.FELADM.FEL_COST_CMPNT_DIM), 0)
...
WHERE NOT EXISTS (
    SELECT 1
    FROM GENDB01.FELADM.FEL_COST_CMPNT_DIM L
